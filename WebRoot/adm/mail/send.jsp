...
<%
	...
	if(title!=null && content!=null && wglist!=null ){
		if(mytext!=null && !mytext.equals("")){
			if(mytext.substring(mytext.length()-1,mytext.length()).equals(",")){
			    mytext=mytext.substring(0, mytext.length()-1);
			}
		}
		if(wglist.substring(wglist.length()-1,wglist.length()).equals(",")){
			wglist=wglist.substring(0, wglist.length()-1);
		}
		String [] usernamelist=wglist.split(",");
		PlayerManager playerManager=PlayerManager.getInstance();
		for(int i=0;i<usernamelist.length;i++){
			Long getiddd=playerManager.getplayerid(usernamelist[i]);
	 		if(getiddd==null){
		...
		String userid=(String)session.getAttribute("userid");
	 	for(int i=0;i<usernamelist.length;i++){
			Long getid=playerManager.getplayerid(usernamelist[i]);
			if(getid!=null){
				MailManager.getInstance().sendmail(Translate.commonmessage5, title, content, mytext, (long)-1, getid , 1,1);
				//增加日志
				WgPlayerserver wg=playerManager.getplayer(getid);
				//playerManager.giveLog2(Integer.valueOf(userid), wg.getUid(),BillingCenter.articletype,0,mytext);
				 /* for(Integer in:mapone.keySet()){
					playerManager.giveLog(Integer.valueOf(userid), wg.getUid(),BillingCenter.articletype,mapone.get(in),String.valueOf(in));
				}  */
			}
		}
	...
	/* if(wglist!=null && mytext!=null){
		...
		String userid=(String)session.getAttribute("userid");
	 	for(int i=0;i<usernamelist.length;i++){
			Long getid=playerManager.getplayerid(usernamelist[i]);
			if(getid!=null){
				MailManager.getInstance().sendmail("GM的邮件", title, content, mytext, (long)-1, getid , 1);
				//增加日志
				WgPlayerserver wg=playerManager.getplayer(getid);
				for(Integer in:mapone.keySet()){
					playerManager.giveLog(Integer.valueOf(userid), wg.getUid(),BillingCenter.articletype,mapone.get(in),String.valueOf(in));
							...
							<div class="am-form-group">
								...
								<div class="am-u-sm-9">
									<select id="myarticle" name="myarticle" data-am-selected="{searchBox: 1,maxHeight: 400}">
									  <c:forEach items="${list }" var="article">
									  			<c:if test="${article.articleType==3}">
									  				<c:if test="${article.subType==1}">
									  					<option value="${article.id }:${article.name }">${article.name }碎片</option>
									  				</c:if>
									  				<c:if test="${article.subType!=1}">
												...
												<c:if test="${article.articleType!=3}">
													...
													<option value="${article.id }:${article.name }">${article.name } 图纸</option>
													</c:if>
