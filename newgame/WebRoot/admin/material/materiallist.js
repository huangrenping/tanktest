   ...
   //注意，这里是模块输出的核心，模块名必须和list时的模块名一致
	//一进入,页面进行全查询
	 ...
	 function getaddall(pageNum,url){
		 queryAll(pageNum,"/material/selectallmaterialpage");
	 }
	 	    ...
	 	    success:function(json){
	 			...
	 			if(json.data && json.data.list.length>0){
	 				...
	 				$.each(pageList,function(index,item){
	 					...
	 					var montyperesult=item.subType;
	 					if(montyperesult==0){
	 						montyperesult="金币"
	 					}else{
	 						montyperesult="钻石"
	 						...
	 						$("#addressbody").append('<tr class="gradeX">'+
	                                 ...
	                                 '<td class="am-text-middle">'+item.ids+'</td>' +
	                                 '<td class="am-text-middle">'+item.drawings+'</td>' +
	 ...
	 function getreceedit(){
	 	...
	 	$(document).on('click','#selectoneuser',function(){
	 		...
	 		layer.open({
	 			...
	 			closeBtn:1,
	 			anim:0,
	 			content:[webPath+'/material/selectonematerial?id='+id],
	 	    	end:function(){
	 	    		getaddall(1);
	 	    	}
	 		});
