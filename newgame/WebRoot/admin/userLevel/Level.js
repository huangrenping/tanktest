   ...
   //注意，这里是模块输出的核心，模块名必须和list时的模块名一致
	//一进入,页面进行全查询
	 ...
	 function getaddall(pageNum){
		 //
		 queayall(pageNum,"/Level/selectAllLevel")
		 
	 	//
	 }
	 
	 //页面查询
	 function queayall(pageNum,url){
		 	layer.load();
		 	var data={};
		 			...
		 			if(json.data && json.data.list.length>0){
		 				...
		 				$.each(pageList,function(index,item){
		 						$("#addressbody").append('<tr class="gradeX">'+
		                                 ...
		                                 '<td class="am-text-middle">'+jsonDateFormat(item.createtime)+'</td>'+
		                                 '<td class="am-text-middle">'+jsonDateFormat(item.updatetime)+'</td>'+ '<td class="am-text-middle">' +
	 ...
	 function addLevel(){
		 $("#myadd").click(function(){
		 		...
		 		layer.open({
		 			...
		 			closeBtn:1,
		 			anim:0,
		 			content:[webPath +"/Level/addLevel",'no'],
		 	    	end:function(){
		 	    		getaddall(1);
		 	    	}
		 		});
	 ...
	 function getreceedit(){
	 	...
	 	$(document).on('click','#selectoneuser',function(){
	 		...
	 		layer.open({
	 			...
	 			closeBtn:1,
	 			anim:0,
	 			content:[webPath+'/Level/selectoneuleverl?id='+id,'no'],
	 	    	end:function(){
	 	    		getaddall(1);
	 	    	}
	 		});
	 	...
	 	$(document).on('click','#deleteoneuser',function(){
	 		...
	 		layer.open({
	 			  ...
	 			  ,yes: function(index, layero){
	 			 		layer.closeAll();
	 			 		$.ajax({
	 				 		url:webPath+"/Level/deleteoneleverl?id="+id,
	 				 	    type:'post',
	 				 	    cache:false,
	 				 	    error : function(XMLHttpRequest, textStatus, errorThrown) {
	 				 			layer.closeAll('loading');
	 ...
	 //按条件查询
	 function selectlevelname(pageNum,url){
		 $("#mybutton").click(function(){
			 var levelname=$("#levelname").val();
			 queayall(pageNum,"/Level/selectlevelname?name="+levelname);
		 })
	 }
