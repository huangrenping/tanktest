<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    web.webSocket.timelimitplot.TimelimitChapter,
    web.webSocket.timelimitplot.TimelimitPlotManager,
...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    <script src="<%=path %>/assets/js/amazeui-pagination.js"></script>
		<%	
			...
			String newtimilimitchartid=request.getParameter("newtimilimitchartid");
			if(datetimepicker1!=null && !datetimepicker1.equals("") &&
			   datetimepicker2!=null && !datetimepicker2.equals("") &&
			   radio1!=null && !radio1.equals("") &&
			   newtimilimitchartid!=null && !newtimilimitchartid.equals("")){
			   ...
			   for(int i=0; i<huodongstr3.length; i++){
			   		...
			   		//id待定，取最后一个
			   		/* if(Integer.valueOf(huodongstr4[0])==11){
			   			huodongstr3[i]="11@"+radio1+"@"+time1+"@"+time2+"@"+0;
			   			activityManager.upWgmainhuodong(Util.Join(huodongstr3, ","));
			   			activityManager.getWgMain().setTimelimitplot(Integer.valueOf(newtimilimitchartid));
			   			break;
			   		} */
			   }
			}
			...
			for(int i=0; i<huodong2.length; i++){
				...
				//id待定，取最后一个
				/* if(Integer.valueOf(huodong3[0])==11){
			...
			int timilimitchartid=activityManager.getWgMain().getTimelimitplot();
			TimelimitPlotManager timelimitPlotManager=TimelimitPlotManager.getInstance();
			TimelimitChapter timelimitChapter=timelimitPlotManager.getChaptermap().get(timilimitchartid);
			List<TimelimitChapter> chapterlist=timelimitPlotManager.getChapterlist();
			request.setAttribute("list", chapterlist);
			request.setAttribute("timilimitchartid", timilimitchartid);
		%>
...
<body class="theme-black">
	...
	<!-- 内容区域 -->
	<form action="uptimelimitplot.jsp" method="post" id="hp_form" name="hp_form" data-am-validator
				...
				<div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
					<div class="widget am-cf">
							...
							<div class="am-form-group">
								<label for="user-name" class="am-u-sm-3 am-form-label">副本名称
									<span class="tpl-form-line-small-"></span>
								</label>
								<div class="am-u-sm-9">
									<select id="newtimilimitchartid" name="newtimilimitchartid" data-am-selected="{searchBox: 1,maxHeight: 400}">
									  <s:if test="${timilimitchartid==0 }">
										  <option value="0" selected="selected"></option>
									  </s:if>
									  <s:forEach items="${list }" var="chapter">
									  			<s:if test="${chapter.id==timilimitchartid }">
									  				<option value="${chapter.id }" selected="selected">${chapter.chaptername }</option>
									  			</s:if>
												<s:if test="${chapter.id!=timilimitchartid }">
