<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    import="web.webSocket.player.PlayerManager,
		...
		if(wg!=null){
			...
			BillingCenter.playerIncrease(wg ,0 , addnum, 1,10);
			wg.setTotalrmbdiamond(wg.getTotalrmbdiamond()+addnum);
			ActivityManager.payall(wg,addnum,10);
			
			Activity2Manager.getInstance().funAddNum(wg, addnum, Activity2Manager.functionNumber1);
			...
			if(Util.isOpenTime(huodong1.getStarttime(), huodong1.getEndtime())){
				ActivityManager.catpay(wg, addnum);//招财猫
				}
			}
			//调用活动充值
			//ActivityManager.getInstance().addpay(wg, 300);
			//wg.setCardtime(wg.getCardtime()+30);
			wg.getWgPlayerserver2().setJikatime(wg.getWgPlayerserver2().getJikatime()+90);
			
			if(wg.getWgPlayerserver2().getJikareceive()==0){
			wg.getWgPlayerserver2().setJikareceive(1);
			wg.getWgPlayerserver2().setDirty(true);
			ActivityManager.getInstance().pushjika(wg);
			
			}
			
			
			int[] type={PlayerManager.jikareceive,PlayerManager.jikatime,PlayerManager.viplvtype};
			//vip礼包红点  累计充值红点
			PlayerManager.getInstance().vipandrechargered(wg);
			wg.pushplayer(type);
			MailManager.getInstance().sendmail(Translate.commonmessage5,Translate.admin_16,Translate.admin_17, "", (long)-1, getid, 1,1);
