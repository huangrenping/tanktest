...
for (WgPlayerserver getplayer : getplayers) {
	if(!getplayer.getPlayername().equals("GM") && !getplayer.getPlayername().equals("系统邮件")){
		...
		if(nowday>=3&&nowday<=6){
			if(Integer.valueOf(strarray[2])==1){//第3天没有签到的补偿10
				num+=10;
			}
			...
			if(Integer.valueOf(strarray[14])==1){//第14天签到的补偿10
				num+=10;
			}
		}else if(nowday>=25&&nowday<=29){
			if(Integer.valueOf(strarray[2])==1){//第3天签到的补偿10
				num+=10;
			}
			if(Integer.valueOf(strarray[6])==1){//第7天签到的补偿5
		...
		if(signtotalstate[2]==2&&nowday<30){//累计签到第三个奖励领取了
			num+=20;
		}
		if(num>0){
