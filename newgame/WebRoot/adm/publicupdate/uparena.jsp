...
if("".equals(type)||type==null){
		...
		if(robot!=null){
			...
			String[] xinglist=xing.split("、");
			String[] minglist=ming.split("、");
			
			String[] names=new String[list.size()];
			for (int i = 0; i <list.size(); i++) {
				names[i]=xinglist[Util.getRandomInt(0, xinglist.length)]+minglist[Util.getRandomInt(0, minglist.length)];	
			}
			for (int i=0 ; i< list.size() ;i++) {
				Arenaplayer arenaplayer=arenaManager.getareanplayer(list.get(i));
				if(arenaplayer!=null){
					if(arenaplayer.getViplevel()>1){
						arenaplayer.setViplevel(1);
					}
					arenaplayer.setPlayername(names[i]);
				}
			}
		}
}
