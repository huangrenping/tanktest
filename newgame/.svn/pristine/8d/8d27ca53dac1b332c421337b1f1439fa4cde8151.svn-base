...
<%
			...
			Map<Integer, Huodong> result=activityManager.getHuodongmap();
			List<Activityorder> list=new ArrayList<Activityorder>();
			for (Huodong hd : result.values()) {
				Activityorder activityorder=new Activityorder();
				if(hd!=null&&Util.isOpenTime(hd.getStarttime(), hd.getEndtime())){
					activityorder.setState("是");
				}else{
					activityorder.setState("否");
				}
				activityorder.setId(hd.getType());
				activityorder.setOrder(hd.getId());
				activityorder.setName(hd.getName());
				activityorder.setStartime(hd.getStarttime());
				activityorder.setEndtime(hd.getEndtime());
