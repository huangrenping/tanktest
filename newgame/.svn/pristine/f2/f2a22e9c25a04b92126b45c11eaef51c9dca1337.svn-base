   ...
   //注意，这里是模块输出的核心，模块名必须和list时的模块名一致
	//一进入,页面进行全查询
	 ...
	 function getaddall(pageNum,url){
		 queryAll(pageNum,"/article/selectallarticlepage");
	 }
	 			...
	 			if(json.data && json.data.list.length>0){
	 				...
	 				$.each(pageList,function(index,item){
	 					...
	 					$("#addressbody").append('<tr class="gradeX">'+
	 								...
	 								'<td class="am-text-middle" style="display: none">'+(item.id)+'</td>' +
	 								'<td class="am-text-middle">'+item.name+'</td>' +
	 								'<td class="am-text-middle">'+item.articleType+'</td>' +
	 								'<td class="am-text-middle">'+item.subType+'</td>' +
	                                 '<td class="am-text-middle">'+item.articleLevel+'</td>' +
	                                 '<td class="am-text-middle">'+item.add+'</td>' +
	                                 '<td class="am-text-middle">'+item.sellnum+'</td>' +
	                                 '<td class="am-text-middle">'+result+'</td>' +
	                                 '<td class="am-text-middle">'+purchase+'</td>' +
	                                 '<td class="am-text-middle">'+item.produce+'</td>' +
	                                 '<td class="am-text-middle">'+item.secondid+'</td>' +
	 ...
	 function getreceiveadd(){
	 	$(document).on('click','#myadd',function(){
	 		...
	 		layer.open({
	 			...
	 			closeBtn:1,
	 			anim:0,
	 			content:[webPath +"/shopping/selectoneshopping",'no'],
	 	    	end:function(){
	 	    		getaddall(1);
	 	    	}
	 		});
	 ...
	 function getreceedit(){
	 	$(document).on('click','#selectoneuser',function(){
	 		...
	 		layer.open({
	 			...
	 			closeBtn:1,
	 			anim:0,
	 			content:[webPath+'/article/selectonearticle?id='+id],
	 	    	end:function(){
	 	    		getaddall(1);
	 	    	}
	 		});
