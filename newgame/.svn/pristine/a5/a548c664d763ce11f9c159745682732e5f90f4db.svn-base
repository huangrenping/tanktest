   ...
   //注意，这里是模块输出的核心，模块名必须和list时的模块名一致
	//一进入,页面进行全查询
	 ...
	 function getaddall(pageNum,url){
		 queryAll(pageNum,"/selectcardpageno");
	 }
	 			...
	 			if(json.data && json.data.list.length>0){
	 				...
	 				$.each(pageList,function(index,item){
	 						$("#addressbody").append('<tr class="gradeX">'+
	                                ...
	                                '<td class="am-text-middle">'+item.serveruid+'</td>' +
	                                '<td class="am-text-middle">'+item.username+'</td>' +
	                                '<td class="am-text-middle">'+item.playername+'</td>' +
	                                '<td class="am-text-middle">'+item.total+'</td>' +
	                                '<td class="am-text-middle">'+item.count3+'</td>' +
	                                '<td class="am-text-middle">'+item.count2+'</td>' +
	                                '<td class="am-text-middle">'+item.count1+'</td>' +
	                                '<td class="am-text-middle">'+item.count0+'</td>' +
	                             '</tr>');   
	 				});
	 				// 显示 开启 分页功能
	 ...
	 function getreceiveadd(){
	 	$(document).on('click','#myadd',function(){
	 		...
	 		layer.open({
	 			...
	 			closeBtn:1,
	 			anim:0,
	 			content:[webPath +"/user/selectoneuser",'no'],
	 	    	end:function(){
	 	    		getaddall(1);
	 	    	}
	 		});
	 ...
	 function getreceedit(){
	 	...
	 	$(document).on('click','#selectoneuser',function(){
	 		...
	 		layer.open({
	 			...
	 			closeBtn:1,
	 			anim:0,
	 			content:[webPath+'/user/selectoneuser?id='+id],
	 	    	end:function(){
	 	    		getaddall(1);
	 	    	}
	 		});
                    ...
                    <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                        <div class="widget am-cf">
                            <div class="widget-head am-cf">
                                <div class="widget-title  am-cf">卡片统计</div>
