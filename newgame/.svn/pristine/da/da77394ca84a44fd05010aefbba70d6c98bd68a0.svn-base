...
<%
			...
			Article article=ArticleManager.getInstance().getarticle(1000235);
			request.setAttribute("articleid", 1000235);
			if(article!=null){
				request.setAttribute("libao", article.getName());
			}
			String huodong=activityManager.getHuodong();
			if(huodong==null || huodong.equals("")){
				//活动id:0关闭或1开启:开始时间:结束时间 :0未发送1已发送
				activityManager.upWgmainhuodong("6@0@0@0@0");
			}else{
				int num=0;
				String[] huodong2=huodong.split(",");
				for(int i=0; i<huodong2.length; i++){
					String[] huodong3=huodong2[i].split("@");
					if(Integer.valueOf(huodong3[0])==6){
						num++;
						break;
					}
				}
				if(num==0){
					activityManager.upWgmainhuodong(huodong+",6@0@0@0@0");
				}
			}
			...
			for(int i=0; i<huodong5.length; i++){
				...
				if(Integer.valueOf(huodong6[0])==6){
	 ...
	 function getoperation(){
	 	$(document).on('click','#operation',function(){
	 		...
	 		layer.open({
	 			...
	 			shadeClose:true,
	 			anim:0,
	 			content:[webPath+'/adm/activity/uptestlibao.jsp'],
	 	    	end:function(){
                    ...
                    <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                        <div class="widget am-cf">
                            <div class="widget-head am-cf">
                                <div class="widget-title  am-cf">封测礼包配置信息</div>
