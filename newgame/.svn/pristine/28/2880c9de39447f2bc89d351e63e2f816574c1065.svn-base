...
<%
			...
			if(huodong==null || huodong.equals("")){
				...
				activityManager.upWgmainhuodong("9@0@0@0@0");
			}else{
				int num=0;
				String[] huodong2=huodong.split(",");
				for(int i=0; i<huodong2.length; i++){
					String[] huodong3=huodong2[i].split("@");
					if(Integer.valueOf(huodong3[0])==9){
						num++;
						break;
					}
				}
				if(num==0){
					activityManager.upWgmainhuodong(huodong+",9@0@0@0@0");
				}
			}
			...
			for(int i=0; i<huodong5.length; i++){
				...
				if(Integer.valueOf(huodong6[0])==9){
					request.setAttribute("id", huodong6[0]);
					request.setAttribute("price", wgMain.getSevendayprice());
...
<%-- 	layui.config({
	 ...
	 function getoperation(){
	 	$(document).on('click','#operation',function(){
	 		...
	 		layer.open({
	 			...
	 			shadeClose:true,
	 			anim:0,
	 			content:[webPath+'/adm/activity/upsevendays.jsp'],
	 	    	end:function(){
	 ...
	 function actreset(){
		 	$(document).on('click','#actreset',function(actreset){
			 	 layer.closeAll();
	 			 layer.confirm('注意:确定清除所有玩家限时七日领奖记录吗?', {icon: 0, title:'一键重置'}, function(index){
				  	location.href =webPath+'/adm/activity/upsevendays.jsp?parameter=1';
				}) 
			})
		};
</script>
                    ...
                    <div class="am-u-sm-12 am-u-md-12 am-u-lg-12">
                        <div class="widget am-cf">
                            <div class="widget-head am-cf">
                                <div class="widget-title  am-cf">七日限时充值配置信息</div>
